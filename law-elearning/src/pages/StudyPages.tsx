...
export const StudyMode = () => {
  ...
  // POPRAWKA: Próba odnalezienia ID artykułu źródłowego (obsługa różnych nazw pól z API)
  const qObj = currentQ as any;
  const linkedArticleId = qObj.artykul_id || qObj.artykul || qObj.article_id || qObj.article;
    ...
    <div className="max-w-2xl mx-auto py-6 perspective-1000">
        ...
        <div className="flex justify-between items-center mb-6">
            <div className="flex flex-col">
                ...
                {linkedArticleId ? (
                    <Link 
                        to={`/artykul/${linkedArticleId}`} 
                        target="_blank"
                        className="text-xs text-indigo-600 hover:text-indigo-800 hover:underline flex items-center mt-1 font-bold"
                    >
                        <span className="mr-1">📖</span> 
                        Kliknij, aby doczytać artykuł źródłowy
                    </Link>
                ) : (
                    <span className="text-[10px] text-gray-400 mt-1 italic">Źródło niedostępne (brak powiązania w bazie)</span>
                )}
            </div>
        ...
        <div className={`mt-10 flex justify-center space-x-6 transition-opacity duration-300 ${isFlipped ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
            ...
            >
                <div className="w-14 h-14 rounded-full bg-red-100 text-red-600 flex items-center justify-center text-xl mb-2 group-hover:bg-red-200 group-hover:scale-110 transition-all border-2 border-transparent hover:border-red-300">✕</div>
            ...
            <button 
                ...
                className="flex flex-col items-center group"
            >
                <div className="w-14 h-14 rounded-full bg-green-100 text-green-600 flex items-center justify-center text-xl mb-2 group-hover:bg-green-200 group-hover:scale-110 transition-all border-2 border-transparent hover:border-green-300">✓</div>
